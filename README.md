...
矛盾标注，因此未清洗测试集上的 98.4% 属于虚高。清洗（去重 1,266 行 + 统一 7 个双标签主题的归属）后，
同一模型在正式测试集上为 **99.53%**（剩余错误的 82.7% 是 NDA/NPS 等英文缩写捷径）；
真实水平以人工评估集（**91.53%**）为准——8.0pp 的差距来自"模板合成 vs 真实口语"的分布差异
与标签体系缺少拒答类，这是答辩的重要亮点而非问题。
