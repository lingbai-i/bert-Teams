...
功能：
- 内置两版系统提示词：v1 基础版（纯零样本）、v2 优化版（增加易混淆类别判定规则），
  用于量化提示词调优的增益；
- 从 test.txt 按固定种子随机采样（默认 200 条），逐条调用 DeepSeek chat 接口，
  记录每条耗时，输出整体准确率与平均延迟；
- 结果与对比记录写入 outputs/llm/metrics.json。

运行方式：
    python -m src.llm_baseline --sample 200

依赖：
- 依赖 src.dataset（复用数据加载）；
